-- Inclui created_at no JSON das ações para a numeração estável A1/A2.
...
comment on view public.current_recommendation_read_model is
  'Situação da recomendação derivada de dispensa, exceção, ações e aceite vigente. Comprovação da execução é opcional. Ações ordenadas pela criação.';
