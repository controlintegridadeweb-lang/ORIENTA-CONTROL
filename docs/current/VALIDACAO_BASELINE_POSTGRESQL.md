...
As **13 evoluções pós-baseline** existentes no repositório atual foram adicionadas depois dessa prova específica. Elas permanecem cobertas pelos gates de migrations e pelo job `Database integration (Supabase local)` do CI; portanto, esta página não deve ser interpretada como evidência de execução remota dessas evoluções.
...
O estado atual possui 10 migrations de baseline + 13 evoluções pós-baseline. Consulte `docs/current/BANCO.md` para o inventário vigente.
