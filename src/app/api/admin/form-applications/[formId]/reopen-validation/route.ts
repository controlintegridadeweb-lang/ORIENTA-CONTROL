...
import { reopenFormApplicationValidation } from "@/features/cycles/server";
...
export const POST = withRoute<{ formId: string }>(
  { roles: ["admin"], route: "/api/admin/form-applications/[formId]/reopen-validation", logMessage: "Failed to reopen form application validation" },
  async ({ request, auth, params }) => {
    ...
    const result = await reopenFormApplicationValidation(createSupabaseServiceRoleClient(), {
      formId,
      ...parsed.data,
