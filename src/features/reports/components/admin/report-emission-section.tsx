...
function reportStatusAccent(status: string): string {
  if (status === "emission_failed") return "border-l-rose-500";
  if (status === "available") return "border-l-emerald-500";
  if (status === "ready_to_emit" || status === "emitting") return "border-l-sky-500";
  return "border-l-slate-300";
}

function reportStatusMessage(status: string): string {
  if (status === "emission_failed") {
    return "A avaliação permanece encerrada. Retome a emissão antes de reabrir o diagnóstico.";
  }
  if (status === "emitting") return "O PDF oficial está sendo processado.";
  if (status === "available") {
    return "A primeira emissão já está preservada; uma nova emissão exige justificativa.";
  }
  if (status === "ready_to_emit") {
    return "O encerramento foi concluído, mas ainda não existe um PDF oficial vigente.";
  }
  return "O relatório oficial ainda não está disponível para este estado do diagnóstico.";
}
      ...
      >
        ...
        {selectedCycle ? (
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            <div
              className={`border-l-4 px-4 py-4 sm:px-5 ${reportStatusAccent(selectedCycle.reportStatus)}`}
              role="status"
            >
              <p className={typography.cardTitle}>
                {REPORT_LIFECYCLE_LABEL[selectedCycle.reportStatus]}
              </p>
              <p className={`mt-1 ${typography.cardDescription}`}>
                {reportStatusMessage(selectedCycle.reportStatus)}
              </p>
            </div>
        ...
        {selectedCycle && selectedCycle.cycleState !== "completed" ? (
          <div
            ...
            role="status"
          >
            <div className="border-l-4 border-l-amber-400 px-4 py-4 sm:px-5">
              <p className={typography.cardTitle}>
                Situação: {cycleStateLabelOrFallback(selectedCycle.cycleState)}
              </p>
              <p className={`mt-1 ${typography.cardDescription}`}>
          ...
          <div className="max-w-3xl overflow-hidden rounded-xl border border-slate-200 bg-white">
            <div className="border-l-4 border-l-amber-400 px-4 py-4 sm:px-5">
              ...
              <textarea
                ...
                className={`${formSurface.input} mt-2 min-h-20`}
              ...
              />
              <p className={`mt-2 ${typography.auxiliary}`}>
